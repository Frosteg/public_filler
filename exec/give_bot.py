...
async def send_smthg():
    ...
    result = cur.fetchone()
    if result == None:
        await bot.send_message(admin_id, "Контент закончился! Необходимо пополнить!")                                #Если реультат выборки None - отправить особщение
                ...
                if group != 1:                                                                      #если количество ссылок больше 1 - формировать media_group
                    ...
                    for row in results_id:                                                            #если категория - Картинки формировать в медиагруп и отправлять с помощью send_group_media
                        ...
                        if p >= 1:
                            ...
                            if p == maxtop:   
                                break                 
                    await bot.send_media_group(channel_id, media=mediagroup)
    ...
    if result_id == 'NotGroup':                                                                #если найдена 1 ссылка в таблице
        ...
        if exp_id == "Photo":                                                             #если категория Картинки
            photo = last_sent
            print(photo)
            await bot.send_photo(channel_id, photo, send)
            ...
            photo = last_sent
            await bot.send_video(channel_id,photo)
            cur.execute(f"UPDATE Content set sent = '{up}' where file_id = '{last_sent}';")#обновление данных таблицы все вхождения по ссылке поста - отметить 1
            conn.commit()
        elif exp_id == "Animation":                                                             #если категория Видео
            photo = last_sent
            await bot.send_animation(channel_id, photo, caption=send)
            cur.execute(f"UPDATE Content set sent = '{up}' where file_id = '{last_sent}';")#обновление данных таблицы все вхождения по ссылке поста - отметить 1
            conn.commit()
        elif exp_id == "Document":                                                             #если категория Видео
            photo = last_sent
            await bot.send_document(channel_id, photo, caption=send)
            cur.execute(f"UPDATE Content set sent = '{up}' where file_id = '{last_sent}';")#обновление данных таблицы все вхождения по ссылке поста - отметить 1
            conn.commit()
        print('Отправлено')        